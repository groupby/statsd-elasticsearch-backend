...
var KEY_NAMES = {
  ...
  groveStats: ["stat", "module", "customerId", "grove"],
...
var es_bulk_insert = function elasticsearch_bulk_insert(listCounters, listTimers, listTimerData) {
  ...
  var insertRequest = http.request(optionsPost, function (res) {
  ...
  ensureIndex(statsdIndex, insertRequest, payload);
};

var ensureIndex = function(statsdIndex, insertRequest, insertPayload){
  var getIndexRequest = {
    host: elasticHost,
    port: elasticPort,
  ...
  var req = http.request(getIndexRequest, function (res) {
    res.on('data', function (d) {
      if (Math.floor(res.statusCode / 100) == 4) {
        console.log("No index found, creating");
        createIndex(statsdIndex, insertRequest, insertPayload);
      } else {
        //console.log("index exists, inserting");
        insertRequest.write(insertPayload);
        insertRequest.end();
      }
    });
  });
...
var createIndex = function(statsdIndex, insertRequest, insertPayload){
  var payload = '{"mappings":{"stat":{"properties":{"@timestamp":{"type":"date","format":"dateOptionalTime"},"value":{"type":"long"}}}}}';
  var postCreateIndex = {
    host: elasticHost,
    port: elasticPort,
    path: elasticPath + statsdIndex,
  ...
  var req = http.request(postCreateIndex, function (res) {
    res.on('data', function (d) {
      if (Math.floor(res.statusCode / 100) == 5) {
        var errdata = "HTTP " + res.statusCode + ": " + d;
        console.log(errdata);
      } else {
        //console.log("index created, inserting");
        insertRequest.write(insertPayload);
        insertRequest.end();
      }
    });
